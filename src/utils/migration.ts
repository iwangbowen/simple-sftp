...
export async function migrateHostConfigsToSettings(
  ...
  if (settingsHosts.length > 0 || settingsGroups.length > 0) {
  ...
  if (!globalStateData || (globalStateData.hosts.length === 0 && globalStateData.groups.length === 0)) {
    logger.info('No data in globalState to migrate');
    return;
  }

  logger.info('=== Starting migration to settings ===');
  logger.info(`Found ${globalStateData.hosts.length} hosts and ${globalStateData.groups.length} groups in globalState`);
